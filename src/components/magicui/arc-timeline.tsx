...
interface DefaultActiveStep {
  time?: string;
  stepIndex?: number;
}

interface ArcTimelineProps extends React.HTMLAttributes<HTMLDivElement> {
  children?: ReactNode;
  data: TimelineData[];
  arcConfig?: ArcConfig;
                  ...
                  <div key={`${lineIndex}-${stepIndex}`}>
                    ...
                    <div
                      className={cn(
                        ...
                        isActive ? "h-[120px] w-[3px]" : "h-16 w-[2px]"
                      )}
                      style={{
        ...
        .map((_, fillIndex) => {
          ...
          return (
            <div
              ...
              className="absolute left-1/2 top-0 h-[34px] w-[1.5px] -translate-x-1/2"
              style={{
                transformOrigin: `50% ${circleWidth / 2}px`,
                transform: `rotate(${fillAngle}deg)`,
              }}>
              <div
                className="h-full w-full bg-gray-300 dark:bg-gray-600"
                style={{
                  transformOrigin: "center top",
                  transform: `rotate(${
